...
throttle("get connected realm list")


with open("./out/connected-realms-list.json", "w") as file:
    file.write(connected_realms_list.json(indent=2))


connected_realms: list[ConnectedRealmResponse] = []
for cr in connected_realms_list.connected_realms:
    realm_res = requests.get(cr.href, headers=build_headers(), params=default_params)

    connected_realm_res = ConnectedRealmResponse.parse_obj(realm_res.json())
    throttle(f"get connected realm {connected_realm_res.id}")

    connected_realms.append(connected_realm_res)

    # with open(
    #     f"./out/connected-realms/{connected_realm_res.id}.json", "w"
    # ) as file:
    #     file.write(connected_realm_res.json(indent=2))

for cr in connected_realms:
    mythic_leaderboards_res = requests.get(
        cr.mythic_leaderboards.href,
